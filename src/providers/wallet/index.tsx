...
import { IconAngleRight, IconSonar } from "@/component/icons";
...
export const WalletContext: FC<PropsWithChildren<{}>> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    stored && connect(stored as Adapter, network, true);

    const chainInfo: ChainInfo = CHAIN_INFO[network];
    DaoDao.connect(chainInfo)
      .then(setWallet)
      .catch((err) => {
        err.message?.includes("missing an account") && toast.error(err.message);
      });
  }, []);

  useEffect(() => {
    wallet && connect(stored as Adapter, network);
  }, [network]);
  ...
  const connect = (adapter: Adapter, chain?: NETWORK, auto?: boolean) => {
    ...
    switch (adapter) {
      case Adapter.Passkey:
        ...
        break;
      case Adapter.Keplr:
        Keplr.connect({ ...chainInfo, rpc }, { feeDenom: feeDenom as string })
          .then((x) => {
            setStored(adapter);
            setWallet(x);
          })
      ...
      case Adapter.Leap:
        Leap.connect({ ...chainInfo, rpc }, { feeDenom: feeDenom as string })
          .then((x) => {
            setStored(adapter);
            setWallet(x);
          })
      ...
      case Adapter.Xfi:
        Xfi.connect({ ...chainInfo, rpc }, { feeDenom: feeDenom as string })
          .then((x) => {
            setStored(adapter);
            setWallet(x);
          })
  ...
  return (
    <Context.Provider key={network + wallet?.account.address} value={value}>
      ...
      <Modal
        ...
        confirm={() =>
          ReadOnly.connect(address as string).then((w) => {
      ...
      >
        <>
          <p className="fs-14 lh-16 color-white mb-2">
            Enter a Kujira address to see a <strong>read only</strong> version
            of the app, as if connected with this address.
          </p>
          <Input
            placeholder="kujira1..."
