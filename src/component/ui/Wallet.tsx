...
import { IconKado } from "@/component/icons/IconKado";
import { IconKeplr } from "@/component/icons/IconKeplr";
import { IconLeap } from "../icons/IconLeap";
import { IconWallet } from "../icons/IconWallet";
import { IconWarning } from "../icons/IconWarning";
import { useNetwork } from "@/providers/network";
import { Adapter, IWallet, useWallet } from "@/providers/wallet";
import { appLink, coinSort } from "@/utils/utils";
...
export function Wallet({
  ...
  const [snapsSupported, setSnapsSupported] = useLocalStorage("snaps", "");
  useEffect(() => {
    getSnaps()
      .then(() => {
        setSnapsSupported("y");
          ...
          <table className="balances">
            <tbody>
              {balances
                ...
                .slice(0, 10)
                .map((b, index) => <Balance balance={b} key={index} />)}
              {balances.length > 5 && (
                <tr>
                  <td className="text-right mono color-grey" colSpan={2}>
                    + {balances.length - 5}
                  </td>
      ...
      {showWalletSelect && (
        <div className="wallet__connections ai-c">
          ...
          <div className="md-flex w-full ai-c jc-c wrap">
            <button
              data-tip={
                snapsSupported ? undefined : "MetaMask Snap support coming soon"
              }
              className={clsx({
            ...
            >
              <img
                src={
                  !snapsSupported ? IconMetamaskDisabled.src : IconMetamask.src
                }
                width="1rem"
                className="mm"
                alt="MetaMask"
...
export const XDefi = () => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 590.031 594.223">
    <g fill="#60FBD0" fillRule="nonzero">
