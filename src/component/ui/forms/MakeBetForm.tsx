...
import { betContract, uskDenom } from "../../../utils/utils";
import {
  SubmitType,
  isDeliverTxResponse,
  isError,
...
export default function MakeBetDialog(props: { bet: BetItem }) {
  const [betAmountText, setBetAmountText] = useState("");
  const [transactionResult, setTransactionResult] = useState<
    "bet-initiated" | DeliverTxResponse | Error | null
  >(null);
  ...
  const [selectedOutcome, setSelectedOutcome] = useState<"" | BetoptionObject>(
    "",
  );
  const [customValueText, setCustomValueText] = useState("");

  useEffect(() => {
    if (transactionResult === "bet-initiated") {
  ...
  const submit = (data: any) => {
    ...
    setTransactionResult("bet-initiated");
  ...
  const onClickBet = async () => {
    ...
    const fundObj = { amount: "" + amountAsString, denom: uskDenom };
    let fundsObj: any[] = [fundObj];
    const selecetedName = selectedOutcome === "" ? "" : selectedOutcome.name;
    let msgObj = {
      bid: { game_idx: props.bet.idx, option: btoa(selecetedName) },
    };
    //let msgObj = {"bid":{"game_idx":props.bet.idx,"option":selectedOutcome}}
    //console.log('msg: ' + JSON.stringify(msgObj))
    //console.log('option: ' + JSON.stringify(selectedOutcome) + ' encoded:' + btoa(selecetedName))
    //console.log('funds: ' + JSON.stringify(fundsObj))
  ...
  const onClickSave = () => {
    ...
    //console.log('Amount: ' + betAmountText + ' on item: ' + selectedOutcome)
    onClickBet();
    setIsOpen(false);
  };
              ...
              >
                <DialogPanel className="w-full bg-gray-700 max-w-md transform overflow-hidden rounded-2xl p-6 text-left align-middle shadow-xl transition-all">
                  ...
                  >
                    <div className="flex w-full">
                      <div className="flex-1 flex text-regular text-white/20">
                        Place a Bet on({props.bet.idx}) {props.bet.title}
                      </div>
                      <div
                  ...
                  <div className="mt-5">
                    ...
                    <div className="flex gap-5 bg-black/10 rounded p-5">
                      <div className="text-white/80 rounded-md bg-black/20 ps-4 pe-2 py-2 text-sm font-medium text-white sm:flex items-center">
                        <TokenImage denom={`usk`} size={20}></TokenImage>{" "}
                        <span className="ms-3">USK</span>{" "}
                        <div className="fill-current ms-1"></div>
                      </div>
                      <input
                        type="text"
                        className="rounded text-black/80 px-2 text-right"
                        onChange={betAmountTextChange}
                        value={betAmountText}
                        inputMode="decimal"
                      ></input>
                    </div>
                    <div className="flex gap-5 bg-black/10 rounded p-5 w-full">
                      <div className="text-white/80 rounded-md bg-black/20 ps-2 pe-2 py-2 text-sm font-medium text-white mx-auto">
                        <RadioGroup
                              ...
                              <div className="flex w-full">
                                <div className="text-sm/6">
                                  ...
                                  <div className="flex gap-2 text-white/50 w-full">
                                    <div className="w-56 mx-auto"></div>
                                  </div>
                                </div>
                    ...
                    <div className="mt-5">
                      <button
                        onClick={() => onClickSave()}
                        className="bg-indigo-500/50 text-lg py-3 w-full rounded-xl hover:bg-indigo-500/70"
                      >
                        Submit
                      </button>
                    </div>
